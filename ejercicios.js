...
function numerosPares(n) {
  ...
  //Inicializo un array vacío, y uso un bucle for que inicie en 2 y finalice hasta n, y aumente de dos en dos.
  // Por cada iteración, se agrega el número par al array usando el push. Retorna mi valor pares.
  const pares = [];
  for(let i=2; i<=n; i+=2){
    pares.push(i);
  }
...
function promedioArray(numeros) {
  ...
  //Inicializo mis variables suma y promedio en 0, y uso un bucle for para recorrer todos los elementos del array. Se suma cada elemento a la variable suma.
  // Y se realiza la división de suma entre la cantidad de elementos del array para obtener el promedio. Retorna el valor del promedio.
  let promedio = 0;
  let suma = 0;
  for(let i = 0; i < numeros.length; i++){
    suma += numeros[i];
...
function encontrarMaximo(numeros) {
  ...
  //Inicializo la variable maximo usando Math.max para encontrar el número más grande de mi array. Y retorno su valor.
...
function filtrarMayores(numeros, limite) {
  ...
  // Incializo mi array vacío y uso un bucle for para recorrer el array. En cada iteración, evalúo si mis números son mayores al límite.
  // Si es así, los agrego mediante push a mi nuevo array. Retorno el array al finalizar el bucle.
  const mayores = [];
  for(let i =0 ; i < numeros.length; i++){
    if(numeros[i] > limite){
...
function invertirArray(arr) {
  ...
  //Uso .reverse() para invertir mi array lo retorno directamente.
  return arr.reverse();
}
 ...
 */
// Uso un bucle for que recorre mi matriz y aumenta un contador. 
// Evalúo si el contador es igual a mi indice indicado de fila, si es así retorno la matriz en esa posición.
...
//Retorno la matriz transpuesta usando el método map que itera sobre las columnas de la matriz, y para cada una se crea una nueva fila con los elementos correspondientes. Formando así la matriz transpuesta.
